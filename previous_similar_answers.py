...
qDataset = pd.read_csv("/Users/zouweijian/Desktop/dataset/dataset/Questions_cleaned.csv", encoding = "ISO-8859-1")
aDataset = pd.read_csv("/Users/zouweijian/Desktop/dataset/dataset/Answers.csv", encoding = "ISO-8859-1")
qDataset = qDataset.sort_values(by="Id", ascending=True)
aDataset = aDataset.sort_values(by="Id", ascending=True)
...
# Create the index directory answers if it doesn't exist
if not os.path.exists("index_ans"):
    os.mkdir("index_ans")
...
# Create the index and add documents to it
ix = index.create_in("index_ans", schema)
writer= ix.writer()

for i, row in aDataset.iterrows():
    Body = row["Body"]
    Id =str(row["Id"])
    writer.add_document(Id=Id, Body=Body)
    if i == 20:
...
def index_search(dirname, search_fields, search_query):
    ...
    with ix.searcher() as searcher:
        ...
        results_dict = {}
        for hit in results:
            results_dict[hit.fields()["Id"]] = hit.fields()["Body"], hit.score
   ...
   #     break
     ...
     #   results = index_search("index_dir", ["Body"], query)
      #  print("Search results:")
       # for Id, (result, score) in results.items():
        #    print(f"Id: {Id}, Body: {result}, Score: {score}\n")
